...
In the search enviroment, the first step of the program is to perform the deserialization. After that, the user can enter a query.  The query is pre-processed, removing accents and lowering the case of letters in upper case. Then, each word in the query is identified and searched in the trie. If that word exists in one or more Wikipedia page, the documents vector containing those pages is returned. If that word does not exist, similar expressions 
